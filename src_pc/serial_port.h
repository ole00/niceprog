...
#define DEFAULT_SERIAL_DEVICE_NAME "/dev/ttyACM0"
...
static void serialDeviceGuessName(char** deviceName) {
    ...
    while (fgets(text, 512, f) != NULL) {
        // filter out non USB devices and prioritise those with Arduino name
        if (CHECK_SERIAL()) {
            int ttyNum  = i;
            int textLen = strlen(text);
            // prefer ESP32 over generic USB serial ports (does not work on OSX)
            if (textLen > 29 && 0 == strncmp(text + 18, "usb-Esp", 7)) {
                ttyNum += 1000;
            }
            if (ttyNum > topTtyNum) {
                topTtyNum = ttyNum;
