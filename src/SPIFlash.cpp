...
SPIFlash::SPIFlash(uint8_t cs, bool overflow) {
	csPin = cs;
    pageOverflow = overflow;
    //ensure SPI and CS pin is initialised in your mian program
}
#endif
...
bool  SPIFlash::readPage(uint16_t page_number, uint8_t *data_buffer, bool fastRead) {
	...
	//read the last byte
    data_buffer[255] = _readNextByte(NO_CONTINUE);
	_endProcess();
	return true;
}
...
bool SPIFlash::unprotectAll()
{
	...
	_cmd(0x01);
	xfer(0x02); //WEL=1, rest of the bits are 0
	CHIP_DESELECT
	if (_notBusy(410L)) {
		return false;
	}
